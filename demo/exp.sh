...
java -Xmx32g -cp libregex/*:libsempre/*:lib/* -ea regex.Train -languageAnalyzer corenlp.CoreNLPAnalyzer -parser SketchParser -executor SketchExecutor -Grammar.inPaths $grammardir -SimpleLexicon.inPaths demo/popl.lexicon -Parser.beamSize $beam -Parser.dumpAllFeatures true -FeatureExtractor.featureDomains rule span context bigram phrase-category  -Dataset.inPaths train:regex/data/$dataset/regex.examples -Learner.batchSize 60 -Learner.maxTrainIters $maxiter  $modeldir
