        ...
        try (BufferedReader brGt = new BufferedReader(new FileReader(new File(spec_filePath)))) {
          ...
          try (BufferedReader br = new BufferedReader(new FileReader(new File(src_filePath)))) {
            ...
            for (String line = br.readLine(); line != null;) {
              ...
              System.out.println(">> Test on sentence: " + currIndx);
              ...
              try {
                ...
                //
                //
                // construct string representations of all candidate derivations generated by sempre 
                // construct count map 
                // construct the top-most predication among all candidates 
                //
                //
                //
                Set<String> derivStringSet = new HashSet<String>();
                Map<String, Integer> derivStringToCount = new HashMap<>();
                String topPrediction = "";
                {
                  ...
                  for (int i = 0; i < derivations.size(); i ++) {
                    ...
                    if (derivStringToCount.containsKey(subDeriv)) {
                      derivStringToCount.put(subDeriv, derivStringToCount.get(subDeriv) + 1);
                    } else {
                      derivStringToCount.put(subDeriv, 1);
                    }

                    // update top predication 
                    if (i == 0) {
                  ...
                  // print and dump 
                  System.out.println(">> All candidates and count: ");
                  System.out.println(">> Count map (size: " + derivStringToCount.size() + ") : ");
                  for (String k : derivStringToCount.keySet()) {
                    int v = derivStringToCount.get(k);
                    System.out.println(">> >> " + k + " : " + v);
                  }

                  System.out.println(">> Ground truth: " + gtLine);
                  System.out.println(">> Top prediction: " + topPrediction);

                  stat.println(currIndx + ": " + "unique deriv size = " + derivStringSet.size());

                }

                assert !derivStringSet.isEmpty();
                ...
                //
                {
                  ...
                  // accuracy 
                  {
                    String s = g.generate(topPrediction);
                    RegExp r = new RegExp(s);
                    Automaton a = r.toAutomaton();
                    if (a.equals(gta)) {
                      accurateCount ++;
